...
class DQNAgent:
    def __init__(self, action_size):
        ...
        self.summary_writer = tf.summary.FileWriter(
            'summary/spaceInvader_dqn', self.sess.graph)
        self.sess.run(tf.global_variables_initializer())

        if self.load_model:
         self.model.load_weights("spaceInvader_dqn.h5")
...
if __name__ == "__main__":
    ...
    for e in range(1):
        ...
        print("Before processing: " + str(np.array(state).shape))
        # plt.imshow(np.array(np.squeeze(state)))
        # plt.show()
        ...
        while not done:
            ...
            env.render()
            # plt.imshow(np.array(np.squeeze(observe)))
            # plt.show()
    ...
    #         env.render()
        ...
        # if e % 1000 == 0:
        #     agent.model.save_weights("spaceInvader_dqn.h5")
